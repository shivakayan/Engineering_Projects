...
//! HTTP resource name
#define HTTP_URL "/update?api_key=S73MGUG1DR3CCURT&field1=0&field2=20&field3=45&field4=29"
...
#define maxph 10.5
#define minph 8
#define APIkey "FW3BW8KYVHP9GF28"
#define minhumid 61
#define maxhumid 80
#define maxtemp 31
#define mintemp 25
...
static void updatestates(int32_t temp,uint32_t humid,uint16_t light_int,float phvalue);
/******************************************************
 *               PIN DEFINITIONS
 ******************************************************/
//connect the peripherals in following manner
//connect fan relay input to pin to P21
//connect humidifier relay input to pin p20
//connect pump relay input to pin p19
...
void updatestates(int32_t temp,uint32_t humid,uint16_t light_int,float phvalue)
{
  uint8_t val;
  sl_status_t status;
  val = 1*(temp>maxtemp) + 2*(humid<minhumid) + 4*((phvalue>maxph)||(phvalue<minph)) + 8*(light_int<luxmin);
  status = SetGpio(val);
  if(status == SL_STATUS_OK)
    DEBUGOUT("\nStatus is good");
  /*
   * temperature -> fan P15;
   * humidity -> Humidifier P16;
   * pH -> Water Pump P17;
   * light intensity -> Onboard LED
   */
//  for(int i=0;i<16;i++){
//      status = SetGpio(i);
//      if(status == SL_STATUS_OK)
...
//  printf("\r\nHTTP Get request init success\r\n");
  ...
  while(1){
    ...
    CLEAN_HTTP_CLIENT_IF_FAILED(status, &client_handle, HTTP_SYNC_RESPONSE);
    printf("\r\nHTTP Get request init success\r\n");
    si70xx_example_process_action(&temp, &humid, &light_int);
    adc_static_mode_example_process_action(&phvalue);
    DEBUGOUT("\nThe recieved phvalue is %lf",phvalue);
    updatestates(temp,humid,light_int,phvalue);
    sprintf(httpurl,"/update?api_key=%s&field1=%ld&field2=%ld&field3=%d&field4=%f",APIkey,temp,humid,light_int,phvalue);
