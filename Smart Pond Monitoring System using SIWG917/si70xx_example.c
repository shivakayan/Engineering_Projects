 ...
 ******************************************************************************/
void si70xx_example_process_action(int32_t *temp, uint32_t *humid, uint16_t *lux)
{
  sl_status_t status;
  uint32_t humidity   = 0;
  ...
  *humid = humidity;
  ...
  status = sl_si91x_BH1750_measure_lux_value(I2C, BH1750_ADDR, &light_intensity);
  if (status != SL_STATUS_OK) {
    DEBUGOUT("Light Intensity read failed, Error Code: 0x%ld \n", status);
  } else {
    DEBUGOUT("\nLight Intensity read is successful\n");
  }
  *temp = temperature;
  *lux = light_intensity;
  DEBUGOUT("\nTemp = %ld, Humidity = %ld, Light Intensity = %d",*temp,*humid,*lux);
}
...
/*******************************************************************************
* Function to measure light intensity using BH1750
*******************************************************************************/
sl_status_t sl_si91x_BH1750_measure_lux_value(sl_i2c_instance_t i2c_instance,
                                                uint8_t addr,
                                                uint16_t *light_intensity)
{
  ...
  status = si70xx_send_command1(i2c_instance, addr, (uint32_t *)light_intensity,BH1750_CMD);
  if (status != SL_STATUS_OK) {
    return status;
  }
  return SL_STATUS_OK;
