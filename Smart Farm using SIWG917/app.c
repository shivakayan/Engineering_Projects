...
#define maxmois 1500
#define minmois 1000
#define APIkey "9SXBOFDUU4DM9LCH" // for changing your API key you have to make changes here
//To change wifi credentials, you have to navigate to config>sl_net_default_values.h and change the ssid and
//password in 2 places
#define minhumid 60
#define maxhumid 80
#define maxtemp 38
#define mintemp 25
...
static void updatestates(int32_t temp,uint32_t humid,uint16_t soilmois);
/******************************************************
 *               PIN DEFINITIONS
 ******************************************************/
...
void updatestates(int32_t temp,uint32_t humid,uint16_t moisvalue)
{
  if(temp>maxtemp)
    setfan();
  else
...
//  printf("\r\nHTTP Get request init success\r\n");
  ...
  while(1){
    ...
    CLEAN_HTTP_CLIENT_IF_FAILED(status, &client_handle, HTTP_SYNC_RESPONSE);
    printf("\r\nHTTP Get request init success\r\n");
    si70xx_example_process_action(&temp, &humid);
    getsoilmoisvalue(&soilmois);
    updatestates(temp,humid,soilmois);
    sprintf(httpurl,"/update?api_key=%s&field1=%ld&field2=%ld&field3=%d",APIkey,temp,humid,soilmois);
